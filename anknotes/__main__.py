...
class Controller:
	...
	repeat with counter_variable_name from 1 to count of myNotes
		...
		repeat with counter from 1 to count of currentAttachments
			...
			set current_filepath to path to temporary items as string

			tell application "Finder"
				try
					make new folder at current_filepath with properties {name:(currentTime as string) & (currentGUID as string)}
				end try
				set current_filepath to current_filepath & (currentTime as string) & (currentGUID as string)
			end tell
